...
const SporkManager = require('../primitives/spork');
...
class Chain extends AsyncEmitter {
  ...
  constructor(options) {
    ...
    this.sporks = this.options.sporks;
    ...
    if(this.sporks)
      this.sporkManager = new SporkManager(this.options);
  ...
  async open() {
    ...
    if(this.sporks && this.sporkManager != null)
      await this.sporkManager.open();
  }
...
class ChainOptions {
  ...
  fromOptions(options) {
    ...
    if(options.sporks != null) {
      assert(typeof options.sporks === 'boolean');
      this.sporks = options.sporks;
    }
