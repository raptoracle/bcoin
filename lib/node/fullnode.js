...
const SporkManager = require('../sporks/spork');
...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    // SPV flag.
    this.spv = false;
    this.slowMode = this.config.bool('slow-mode', false);
    this.sporks = this.config.bool('sporks');
    this.sporksManager = this.sporks ? new SporkManager({
      network: this.network,
      logger: this.logger,
      prefix: this.config.prefix,
      memory: this.config.bool('memory'),
      flushInterval: this.config.uint('flush-interval'),
    }) : null;

    this.title = this.config.str('title', 'bcoin');

    // Instantiate block storage.
    this.blocks = blockstore.create({
      db: this.config.str('db', 'leveldb'),
      dbExclusive: this.config.bool('db-exclusive', false),
      dbExternal: this.config.bool('db-external', false),
      dbName: this.config.str('db-name', `${this.title}-${this.network.type}`),
      dbHost: this.config.str('db-host', 'localhost'),
      dbPort: this.config.uint('db-port', 0),
      dbUser: this.config.str('db-user', null),
      dbPass: this.config.str('db-pass', null),
      network: this.network,
      logger: this.logger,
      prefix: this.config.prefix,
    ...
    this.chain = new Chain({
      ...
      indexFull: this.config.bool('index-full'),
      indexTX: this.config.bool('index-full') || this.config.bool('index-tx'),
      indexAddress: this.config.bool('index-full') || this.config.bool('index-address'),
      sporksManager: this.sporksManager,
    });
    ...
    // Indexers
    if (this.config.bool('index-full') || this.config.bool('index-tx')) {
    ...
    if (this.config.bool('index-full') || this.config.bool('index-address')) {
...
/*
    if (this.config.bool('index-full') || this.config.bool('index-filter')) {
...
*/
  ...
  async getAddressBalance(addr) {
    if(this.addrindex) {
      ...
      let value = 0;
      const bal = await this.addrindex.getAddressBalance(addr);
      if(!!bal)
        value = new BN(bal);

      return Amount.btc(value);
    }
  }
