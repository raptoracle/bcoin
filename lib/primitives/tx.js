...
class TX {
  ...
  /**
   * Get all input values summed by address.
   * @private
   * @param {CoinView} view
   * @returns {Array} [addrs, table]
   */

  _getInputValueByAddress(view) {
    const iAddrs = [];

    if (this.isCoinbase() || this.isQuorumCommitment())
      return iAddrs;
    ...
    for (const input of this.inputs) {
      ...
      const m = _.find(iAddrs, {address: hash});
      const spent = new BN(coin.value);
      ...
      if(!m) {
        ...
        const i = _.findIndex(iAddrs, {address: hash});
        iAddrs[i].spent = iAddrs[i].spent.add(spent);
      }
    }
  ...
  _getOutputValueByAddress() {
    const oAddrs = [];

    if (this.isQuorumCommitment())
      return oAddrs;
    ...
    for (const output of this.outputs) {
      ...
      const hash = addr.getHash('hex');
      const m = _.find(oAddrs, {address: hash});
      const received = new BN(output.value);
      ...
      if(!m) {
        ...
        const i = _.findIndex(oAddrs, {address: hash});
        oAddrs[i].received = oAddrs[i].received.add(received);
      }
    }
  ...
  _getAddressesValues(view) {
    const iAddrs = this.getInputAddressesValues(view);
    const oAddrs = this.getOutputAddressesValues();

    const addrValues = [];

    const zero = new BN(0);

    _.forEach(oAddrs, (addr) => {
      const m = _.find(addrValues, {address: addr.address});
      ...
      if(!m) {
        ...
        const i = _.findIndex(addrValues, {address: addr.address});
        addrValues[i].received = addrValues[i].received.add(received);
        addrValues[i].delta = addrValues[i].delta.add(received);
      }  
    });
    
    _.forEach(iAddrs, (addr) => {
      const m = _.find(addrValues, {address: addr.address});
      ...
      if(!m) {
        ...
        const i = _.findIndex(addrValues, {address: addr.address});
        addrValues[i].spent = addrValues[i].spent.add(spent);
        addrValues[i].delta = addrValues[i].delta.sub(spent);
      }
    });
  ...
  getInputAddressesValues(view) {
    const [iAddrs] = this._getInputValueByAddress(view);
    return iAddrs;
  }
  ...
  getOutputAddressesValues() {
    const [oAddrs] = this._getOutputValueByAddress();
    return oAddrs;
  }
  ...
  getAddressesValues(view) {
    const [addrValues] = this._getAddressesValues(view);
    return addrValues;
  }
   ...
   * @param {CoinView} view
   * @param {Number} height
   * @param {Number|BN?} size - Size to calculate priority
  ...
  getPriority(view, height, size) {
    ...
    for (const {prevout} of this.inputs) {
      ...
      if (coinHeight <= height) {
        const age = new BN(height - coinHeight);
        const cv = new BN(coin.value);
        //sum += BigInt(coin.value) * BigInt(age);
        sum.iadd(cv).imul(age);
      }
    }

    //let ss = sum.idiv(size).toNumber();

    return sum.idiv(size);
