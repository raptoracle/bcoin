...
class SporkMessage {
  ...
  getJson() {
    return {
      ...
      time: this.time,
      active: this.timeSigned < this.time,
      hash: this.hash(),
...
class SporkManager {
  ...
  constructor(options) {
    this.options = new SporkManagerOptions(options);
    this.time = new TimeData();
    this.network = this.options.network;
    this.logger = this.options.logger.context('spork manager');
  ...
  process(packet) {
    ...
    //if(Object.keys(SporkMessage.typesByVal).includes(spork.id)) {
      this.add(spork);
    //} else {
      //bad spork! 
    //}
 ...
 */
  ...
  async open() {
    this.logger.info('Setting up sporks manager.');
    try {
      await this.loadFile();
    } catch (e) {
  ...
  start() {
    ...
    // wait 10 seconds and see if we collect any spork packets.
    setTimeout(() => {
      if(this.needsFlush)
        this.flush();
    }, 10 * 1000);
  ...
  /**
   * Read and initialize from sporks file.
   * @method
   * @returns {Promise}
   */
  ...
  add(spork) {
    ...
    if(entry) {
      ...
      if (entry.timeSigned && spork.timeSigned <= entry.timeSigned)
        return false;
      ...
      entry.sig = spork.sig;
      entry.active = this.isActive(spork);
      entry.hash = spork.hash;
    } else {
      entry = spork;
    }
  ...
  isActive(spork) {
    if(spork.timeSigned < this.network.now() || spork.value !== SporkMessage.defaults[SporkMessage.typesByVal[spork.id]]) {
      return true;
    }
    return false;
  }
  ...
  isSpecialFeesActive() {
    const specialFeeSpork = SporkMessage.types['SPORK_22_SPECIAL_TX_FEE'];
    let entry = this.map.get(specialFeeSpork);
    if(entry) {
      if(entry.timeSigned < this.network.now() && entry.value !== SporkMessage.defaults['SPORK_22_SPECIAL_TX_FEE']) {
        return entry.value;
      }
    }
    return 0;
  ...
  getSpecialFeeValue() {
    return this.isSpecialFeesActive();
  }
...
/**
 * Spork Manager Options
 * @alias module:sporks.SporkManagerOptions
 */

class SporkManagerOptions {
  /**
   * Create spork manager options.
   * @constructor
   * @param {Object?} options
   */
