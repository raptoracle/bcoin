...
exports.encode = function encode(num, exp) {
  assert((Number.isSafeInteger(num) || typeof num === 'bigint' || BN.isBN(num)), 'Invalid number value.');
  let _num = new BN(num);
  ...
  let lo = _num.mod(mult);
  let hi = _num.isub(lo).div(mult);
...
exports.decode = function decode(str, exp) {
  ...
  //const decodedNum = sign * (hi * mult + lo);
