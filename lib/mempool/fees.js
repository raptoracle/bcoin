...
*/
    this.logger.debug('target %d, %s breakpoint %d, decay: %d, median %d', 
      target, greater ? ">" : "<", breakpoint, this.decay, median,
    );

    console.log("passbucket", _passBucket);
    console.log("failbucket", _failBucket);
  ...
  getMaxConfirms() {
    return this.scale * this.maxConfirms;
  }
  ...
  fromRaw(data) {
    ...
    let maxPeriods = scale * maxConfirms;
    ...
    if (maxPeriods <= 0 || maxPeriods > 6 * 24 * 7)
      throw new Error('Must maintain estimates for between 1 and 1008 (one week) confirms.');
...
class PolicyEstimator {
  ...
  estimateFee(target, smart, conservative) {
    ...
    if (smart == true)
      return this.estimateSmartFee(target, conservative);

    if(conservative == true)
      return this.estimateConservativeFee(target * 2);
 ...
 */
  ...
  estimateCombinedFee(target, successThreshold, checkShorterHorizon) {
    ...
    console.log("estimateCombinedFee ", estimate);
    return estimate;
  }
 ...
 */
  ...
  estimateConservativeFee(target) {
    ...
    console.log("estimateConservativeFee", estimate);
    return estimate;
  }
  ...
  estimateSmartFee(target, conservative) {
    ...
    console.log("estimatesmartfee, 1037 ");
    ...
    console.log("estimatesmartfee 1080", median);
