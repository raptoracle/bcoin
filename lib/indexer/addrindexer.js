...
Object.assign(layout, {
  ...
  I: bdb.key('I', ['uint8', 'hash', 'uint32', 'uint32']), // timestamp incoming - prefix,hash,height,timestamp -> value
  S: bdb.key('S', ['uint8', 'hash', 'uint32', 'uint32']), // timestamp spent
  B: bdb.key('B', ['uint8', 'hash', 'uint32', 'uint32']), // timestamp balance
...
class AddrIndexer extends Indexer {
  ...
  async indexBlock(meta, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      let hasInputAddress = false;
      let hasOutputAddress = false;
      ...
      for (const {address, value} of tx.getInputAddressesValues(view)) {
        const prefix = address.getPrefix(this.network);
        if (prefix < 0)
          continue;
        const addrHash = address.getHash();
        amountSpent[addrHash] += value;
        //this.put(layout.S.encode(prefix, addrHash, height, i), null);
        hasInputAddress = true;
      }

      for (const {address, value} of tx.getOutputAddressesValues(view)) {
        const prefix = address.getPrefix(this.network);
        if (prefix < 0)
          continue;
        const addrHash = address.getHash();
        amountIn[addrHash] += value;
        //this.put(layout.A.encode(prefix, addrHash, height, i), null);
    ...
    for (const [address, value] of amountSpent) {
      const prefix = address.getPrefix(this.network);
      if (prefix < 0)
        continue;

      this.put(layout.S.encode(prefix, address, height, time), value);
      balances[address] -= value;
    }

    for (const [address, value] of amountIn) {
      const prefix = address.getPrefix(this.network);
      if (prefix < 0)
        continue;

      this.put(layout.I.encode(prefix, address, height, time), value);
      balances[address] += value;
    }
  }
