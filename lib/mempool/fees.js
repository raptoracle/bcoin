...
class ConfirmStats {
  ...
  estimateMedian(target, needed, breakpoint, greater, height) {
    ...
    for (let bucket = start; bucket >= 0 && bucket <= max; bucket += step) {
      ...
      if (total >= needed / (1 - this.decay)) {
        ...
        // and reset the counters
          ...
          found = true;
          passing = true;
          passBucket.withinTarget = conf;
          conf = 0;
          passBucket.totalConfirmed = total;
          total = 0;
          passBucket.inMempool = extra;
          passBucket.leftMempool = fail;
          fail = 0;
          extra = 0;
          bestNear = near;
          bestFar = far;
    ...
    if(passing && !newBucketRange) {
      ...
      failBucket.withinTarget = conf;
      failBucket.totalConfirmed = total;
      failBucket.inMempool = extra;
      failBucket.leftMempool = fail;
    }
