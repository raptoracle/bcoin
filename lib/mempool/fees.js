...
class ConfirmStats {
  ...
  estimateMedian(target, needed, breakpoint, greater, height) {
      ...
      if (total >= needed / (1 - this.decay)) {
        ...
        if (greater && perc < breakpoint || (!greater && perc > breakpoint)) {
          if(passing == true) {
            ...
            this.failBucket.start = failMinBucket ? this.buckets[failMinBucket - 1] : 0;
            this.failBucket.end = this.buckets[failMaxBucket];
            this.failBucket.withinTarget = conf;
            this.failBucket.totalConfirmed = total;
            this.failBucket.inMempool = extra;
            this.failBucket.leftMempool = fail;
            passing = false;
          }
          continue;
        } else {
          ...
          found = true;
          passing = true;
          this.passBucket.withinTarget = conf;
          conf = 0;
          this.passBucket.totalConfirmed = total;
          total = 0;
          this.passBucket.inMempool = extra;
          extra = 0;
          this.passBucket.leftMempool = fail;
          fail = 0;
          bestNear = near;
          bestFar = far;
          //near = i + step;
    ...
    if (found && sum !== 0) {
      ...
      this.passBucket.start = minBucket ? this.buckets[minBucket-1] : 0;
      this.passBucket.end = this.buckets[maxBucket];
    }
  ...
  fromRaw(data) {
    ...
    this.maxPeriods = confAvg.length;
    this.maxConfirms = scale * maxPeriods;

    if(this.maxConfirms <= 0 || this.maxConfirms > 6 * 24 * 7)
    ...
    if (this.maxPeriods != failAvg.length)
    ...
    this.init(buckets, this.maxPeriods, decay, scale);
...
class PolicyEstimator {
  ...
  reset() {
    this.feeUnlikely = 0;
    this.feeLikely = pConstants.INF_FEERATE;
  ...
  isFeePoint(fee) {
    if (fee >= pConstants.MIN_BUCKET_FEERATE || fee > pConstants.INF_FEERATE)
      return true;
