...
class AddrIndexer extends Indexer {
  ...
  async indexAddressBalance(meta, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if(Object.entries(addressValues).length < 1)
        continue;

      for (const [addr, value] of Object.entries(addressValues)) {
        let add = Address.fromString(addr);
        const hash = add.getHash();
        const prefix = add.getPrefix(this.network);
        if (prefix < 0)
          continue;
        if (!table.has(hash)) {
          table.add(hash);
          txValues.push({[addr]: value});
        } else {
          let prevValues = txValues.findIndex(e => e[addr] === addr);
          if(prevValues > -1) {
            let newValues = { received: txValues[prevValues][addr].received + value.received, spent: txValues[prevValues][addr].spent + value.spent };
            txValues[prevValues][addr] = newValues;
          }
        }
      }
