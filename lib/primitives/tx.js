...
class TX {
  ...
  _getInputValueByAddress() {
    const table = new BufferSet();
    const iValues = [];

    if (this.isCoinbase())
      return [table, iValues];
  ...
  _getInputAddressesValues(view) {
    ...
    for (const input of this.inputs) {
      ...
      if (!table.has(hash)) {
        table.add(hash);
        addrs.push({address: addr, value: coin.value});
      }
    }
  ...
  /**
   * Get all output addresses values.
   * @private
   * @returns {Array} [addrs, table]
   */

  _getOutputAddressesValues() {
    const table = new BufferSet();
    const addrs = [];
    ...
    for (const output of this.outputs) {
      ...
      if (!table.has(hash)) {
        table.add(hash);
        addrs.push({address: addr, value: output.value});
      }
    }
  ...
  getInputAddressesValues(view) {
    const [addrs] = this._getInputAddressesValues(view);
    return addrs;
  }
  ...
  getOutputAddressesValues() {
    const [addrs] = this._getOutputAddressesValues();
    return addrs;
  }
