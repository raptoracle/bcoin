...
Object.assign(layout, {
  ...
  M: bdb.key('M', ['hash', 'hash256', 'uint32']),
});
      ...
      if(this.mongo.enabled) {
        if (!tx.isCoinbase()) {
          for (const { prevout } of tx.inputs) {
            ...
            await this.mdb.removeAddress(layout.M.encode(addr, prevout.hash, prevout.index));
          }
        }
        ...
        for (let i = 0; i < tx.outputs.length; i++) {
          ...
          await this.mdb.saveAddress(layout.M.encode(addr, hash, i));
        }
      } else {
      ...
      if(this.mongo.enabled) {
        if (!tx.isCoinbase()) {
          for (const { prevout } of tx.inputs) {
            ...
            await this.mdb.saveAddress(layout.M.encode(addr, prevout.hash, prevout.index));
          }
        }
        ...
        for (let i = 0; i < tx.outputs.length; i++) {
          ...
          await this.mdb.removeAddress(layout.M.encode(addr, hash, i));
        }
      } else {
        let hasAddress = false;
