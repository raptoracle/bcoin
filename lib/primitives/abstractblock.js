...
class AbstractBlock {
  ...
  /**
   * Hash the block headers with configured algo.
   * @param {String?} enc - Can be `'hex'` or `null`.
   * @returns {Hash|Buffer} hash
   */

  powHash() {
    return multihashing[common.algorithm](this.toHead());
  }
  ...
  verifyPOW(network) {
    return consensus.verifyPOW(this.powHash(), this.bits, network);
  }

  /**
   * Verify genesis blocks proof-of-work.
   * @returns {Boolean}
   */

  verifyGenesisPOW(network) {
    return consensus.verifyGenesisPOW(this.powHash(), this.nonce, network);
  }
  ...
  getPOWHash() {
    return util.revHex(this.powHash());
  }
