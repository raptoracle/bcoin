...
class Output {
  ...
  fromOptions(options) {
    ...
    if (options.value) {
      assert((Number.isSafeInteger(options.value) || typeof options.value === 'bigint') && options.value >= 0,
  ...
  fromScript(script, value) {
    ...
    assert((Number.isSafeInteger(value) || typeof value === 'bigint') && value >= 0,
      'Value must be a safe integer or bigint.');
