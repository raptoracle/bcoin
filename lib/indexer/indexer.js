...
const {ZERO_HASH} = require('../protocol/params/consensus');
...
class Indexer extends EventEmitter {
  ...
  async open() {
    ...
    if(this.mongo.enabled) {
      await this.mdb.open();
      const data = await this.mdb.getTipHash();

      if (data)
        this.height = await this.mdb.getBlockHeightByHash(data);
      else 
        this.height = 0;
  ...
  async saveGenesis() {
    ...
    const meta = new BlockMeta(block.hash(), 0, block.time);
  ...
  async getBlockMeta(height) {
    const data = this.mongo.enabled ? await this.mdb.getEntryHashByHeight(height) : await this.db.get(layout.h.encode(height));

    if (!data)
      return null;

    return new BlockMeta(data, height, data.time);
  }
  ...
  async _rollforward() {
    ...
    for (let height = this.height + 1; ; height++) {
      ...
      const meta = new BlockMeta(entry.hash, height, entry.time);
...
class BlockMeta {
  constructor(hash, height, time) {
    ...
    assert(Number.isInteger(this.time));
  }
}
