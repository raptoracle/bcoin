...
class MempoolEntry {
  ...
  getPriority(height) {
    const delta = Number(height - this.height);
    let ts = new BN(this.size);
    //const priority = (delta * this.value) / this.size;
    const priority = this.value.imuln(delta).idiv(ts).toNumber();

    //let result = this.priority + Math.floor(priority);
    let result = Number(this.priority) + Math.floor(Number(priority));
