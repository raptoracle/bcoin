...
class TX {
  ...
  _getInputValueByAddress(view) {
    ...
    for (const input of this.inputs) {
      ...
      const value = new BN(coin.value).toBigInt();
  ...
  _getOutputValueByAddress() {
    ...
    for (const output of this.outputs) {
      ...
      const hash = addr.getHash();
      const value = new BN(output.value).toBigInt();
  ...
  _getAddressesValues(view) {
    ...
    const addrs = _.merge(Object.keys(iAddrs), Object.keys(oAddrs));
    const addrValues = [];

    for (const addr of addrs) {
      let str = addr.toString();
      addrValues.push({address: str, received: oAddrs[str] ??= 0, spent: iAddrs[str] ??= 0 });
    }
