...
const commonMining = require('./common');
...
class BlockTemplate {
  ...
  setBits(bits) {
    ...
    this.bits = bits;
    this.target = commonMining.getTarget(bits);
  }
  ...
  setTarget(target) {
    ...
    this.bits = commonMining.getBits(target);
    this.target = target;
  }
  ...
  getProof(nonce1, nonce2, time, nonce) {
    ...
    const hash = multihashing[common.algorithm](data);
  ...
  getDifficulty() {
    return commonMining.getDifficulty(this.target);
  }
...
class BlockProof {
  ...
  verify(target) {
    return commonMining.rcmp(this.hash, target) <= 0;
  }

  getDifficulty() {
    return commonMining.getDifficulty(this.hash);
  }
}
