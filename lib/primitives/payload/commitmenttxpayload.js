...
class CommitmentTxPayload extends AbstractPayload {
  ...
  toBuffer(options) {
    ...
    payloadBufferWriter
      ...
      .writeHash(Buffer.from(this.quorumPublicKey, 'hex'))
      .writeHash(Buffer.from(this.quorumVvecHash, 'hex'))
      .writeHash(Buffer.from(this.quorumSig, 'hex'))
      .writeHash(Buffer.from(this.sig, 'hex'));
