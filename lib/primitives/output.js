...
const policy = require('../blockchain/params/policy');
...
class Output {
  ...
  fromOptions(options) {
    ...
    if (options.value) {
      //assert((Number.isSafeInteger(options.value) || typeof options.value === 'object') && options.value >= 0,
  ...
  fromScript(script, value) {
    ...
    //assert((Number.isSafeInteger(value) || typeof value === 'object') && value >= 0,
    //  'Value must be a safe integer or bigint.');
  ...
  compare(output) {
    ...
    const cmp = new BN(this.value.isub(output.value));
  ...
  fromJSON(json) {
    ...
    //assert((Number.isSafeInteger(json.value) || typeof json.value === 'bigint') && json.value >= 0,
  ...
  toWriter(bw) {
    bw.writeBigI64(this.value.toBigInt());
  ...
  fromReader(br) {
    this.value = new BN(br.readBigI64());
