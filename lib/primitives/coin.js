...
class Coin extends Output {
 ...
 [inspectSymbol]() {
  ...
  fromJSON(json) {
    ...
    //  'Value must be a safe integer.');
    assert((!Number.isNaN(json.value) && json.value >= 0) || BN.isBN(json.value),
      'Value must be a uint64 or BN.');
  ...
  toWriter(bw) {
    assert(BN.isBN(this.value), "coin.js toWriter(), value expected to be BN");
  ...
  fromTX(tx, index, height) {
    ...
    this.height = height;
    this.value = new BN(tx.outputs[index].value);
