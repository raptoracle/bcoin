...
class HTTP extends Server {
  ...
  constructor(options) {
    ...
    this.sporksManager = this.node.chain.sporksManager;
  ...
  initRouter() {
    ...
    this.get('/tx/address/:address', async (req, res) => {
      ...
      const ids = valid.bool('ids', false);
      ...
      for (const meta of metas) {
        const view = await this.node.getMetaView(meta);
        const json = meta.getJSON(this.network, view, this.chain.height);

        if(ids || !verbose) {
          result.push(json.hash);
        } else {
          result.push(json);
        }
      }
    ...
    // Asset by hash
    this.get('/asset/:hash', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.brhash('hash');
    ...
    // Balance by address
    this.get('/address/balance/:address', async (req, res) => {
      const valid = Validator.fromRequest(req);
      let address = valid.str('address');

      let isArray = address.includes(',');
      ...
      if(isArray) {
        ...
        for await (const address of addresses) {
          const _address = address.replace(/[^0-9a-zA-Z,]/gi, '');
          const balance = await this.node.getAddressBalance(_address);
          results.push({address: _address, balance: balance});
        }

      } else {
        let balance = await this.node.getAddressBalance(address);
        if (!balance) {
          res.json(404);
          return;
        }
        results = {address: address, balance: balance};
      }
    ...
    this.get('/mempool', async (req, res) => {
      ...
      const hashes = this.mempool.getSnapshot(verbose);
      const result = [];
    ...
    // Mempool by address
    this.get('/mempool/address/:address', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const address = valid.str('address');
      const limit = valid.uint('limit', this.options.maxTxs);
      const reverse = valid.bool('reverse', false);
      ...
      const metas = await this.mempool.getMetaByAddress(
        addr, {limit, reverse, after});
    ...
    this.get('/fee', async (req, res) => {
      ...
      const smart = valid.bool('smart', true);
      const conservative = valid.bool('conservative', true);
      ...
      const fee = this.fees.estimateFee(blocks, smart, conservative);
      const result = fee > 0 ? fee : this.network.minRelay;

      res.json(200, { rate: result });
    });

    // Estimate fees
    this.get('/fees', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const blocks = valid.u32('blocks', 1);
      const smart = valid.bool('smart', true);
      const conservative = valid.bool('conservative', true);

      const result = { rate: this.network.feeRate };
      ...
      const specialFee = this.sporksManager.getSpecialFeeValue();

      result.fee = fee > 0 ? fee : this.network.minRelay;
      result.feeSatsPerKb = this.network.feeRate / 1000;
      result.relayFee = this.network.minRelay;
      result.incrementalFee = this.network.minRelay;
      if(specialFee > 0) 
        result.specialFee = specialFee * 1e8;
