...
class ChainDB {
  ...
  async open() {
    ...
    this.logger.info(
      ...
      this.state.tx,
      this.state.coin,
      Amount.btc(new BN(this.state.value)));
  }
...
class ChainState {
  ...
  add(coin) {
    this.coin += 1;
    this.value.add(new BN(coin.value));
    //this.value += BigInt(coin.value);
  }

  spend(coin) {
    this.coin -= 1;
    this.value.sub(new BN(coin.value));
    //this.value -= BigInt(coin.value);
  }
