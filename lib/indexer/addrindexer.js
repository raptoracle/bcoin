...
class AddrIndexer extends Indexer {
  ...
  async indexAddressBalance(meta, block, view) {
    ...
    txValues.forEach((entry, index) => {
      ...
      this.updateAddressBalance(entry.address, spent, received);
    });
  }
  ...
  async unindexAddressBalance(meta, block, view) {
    ...
    txValues.forEach((entry, index) => {
      ...
      this.db.del(layout.Q.encode(prefix, hash));
    });
  }
  ...
  async updateAddressBalance(addr, spent, received) {
    const address = Address.fromString(addr, this.network);
    const hash = address.getHash(addr);
    const prefix = address.getPrefix();

    let balance = await this.getAddressBalance(addr);

    balance = new BN(balance);

    balance.isub(spent);
    balance.iadd(received);

    //await this.db.del(layout.Q.encode(prefix, hash));

    await this.db.put(layout.Q.encode(prefix, hash), fromBigU64(balance.toBigInt()));
  }
}
