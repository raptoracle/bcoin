/*!
 * blockstore/external.js - external blockstore for bcoin
 ...
 *
 * @alias module:blockstore:ExternalBlockStore
 * @abstract
 */

class ExternalBlockStore extends AbstractBlockStore {
  /**
   * Create a blockstore that stores blocks in Mongo.
   * @constructor
   */
  ...
  async open() {
    this.logger.info('Opening External BlockStore...');

    await this.db.open();
    //await this.db.verify(layout.V.encode(), 'mongoblockstore', 0);
  }
  ...
  async close() {
    this.logger.info('Closing External BlockStore...');

    await this.db.close();
  }

  /**
   * This method stores merkle block data in Mongo.
  ...
  /**
   * This method stores block undo coin data in Mongo.
   * @param {Buffer} hash - The block hash
   * @param {Buffer} data - The block data
   * @returns {Promise}
   */
  ...
  /**
   * This method stores block data in Mongo.
   * @param {Buffer} hash - The block hash
   * @param {Buffer} data - The block data
   * @returns {Promise}
   */
  ...
  /**
   * This method stores serialized block filter data in Mongo.
   * @param {Buffer} hash - The block hash
...
module.exports = ExternalBlockStore;
