...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getaddressbalance', this.getAddressBalance);
  }
  ...
  async getTXOut(args, help) {
    ...
    return {
      ...
      value: new BN(coin.value).toNumber(),
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      sigoplimit: consensus.getMaxBlockSigOps(),
      sizelimit: consensus.getMaxBlockSize(),
  ...
  async getAddressBalance(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, `getaddressbalance '{"addresses": ["RTtyQU6DoSuNWetT4WUem5qXP5jNYGpwat"]}'`);

    const valid = new Validator(args);
    const data = valid.obj(0);

    if(!data)
      throw new RPCError(errs.INVALID_PARAMETER, 'Invalid json string.');

    if(data.addresses) {
      const addrs = data.addresses;
      for (let i = 0; i < addrs.length; i++) {
        const addr = parseAddress(addrs[i], this.network);
        const meta = await this.node.getTXByAddress(addr);
      }
    }
