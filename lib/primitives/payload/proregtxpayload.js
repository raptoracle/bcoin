...
const {BLS_PUBLIC_KEY_SIZE, BLS_SIGNATURE_SIZE, EMPTY_SIGNATURE_SIZE, PUBKEY_ID_SIZE} = require('../common').constants;
...
class ProRegTxPayload extends AbstractPayload {
  ...
  validate() {
    ...
    if (this.scriptPayout) {
      ...
      //assert(script.isPublicKeyHashOut() || script.isScriptHashOut(), 'Expected scriptPayout to be a p2pkh/p2sh');
    }
  ...
  toBuffer(options) {
    ...
    payloadBufferWriter
      ...
      .writeU16(this.type)
      .writeU16(this.mode)
      .writeHash(Buffer.from(this.collateralHash, 'hex').reverse())
      .writeI32(this.collateralIndex)
      .writeBytes(ipUtils.ipAndPortToBuffer(this.service))
      .writeHash(Buffer.from(this.keyIDOwner, 'hex').reverse())
      .writeHash(Buffer.from(this.pubKeyOperator, 'hex'))
      .writeHash(Buffer.from(this.keyIDVoting, 'hex').reverse())
      .writeU16(this.operatorReward)
      .writeVarint(Buffer.from(this.scriptPayout, 'hex').length)
      .writeHash(Buffer.from(this.scriptPayout, 'hex'))
