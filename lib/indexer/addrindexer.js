...
class AddrIndexer extends Indexer {
  ...
  async indexAddressBalance(meta, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      addressValues.forEach((entry, index, array) => {
        const findKey = _.findIndex(txValues, { 'address': entry.address });
        if(findKey > -1) {
          let pReceived = new BN(txValues[findKey].received);
          let tReceived = new BN(entry.received);
          let pSpent = new BN(txValues[findKey].spent);
          let tSpent = new BN(entry.spent);
          let cReceived = pReceived.iadd(tReceived).toBigInt();
          let cSpent = pSpent.iadd(tSpent).toBigInt();
          let newValues = {address: entry.address, received: cReceived, spent: cSpent};
          txValues[findKey] = newValues;
        } else {
          let newValues = {address: entry.address, received: entry.received, spent: entry.spent};
          txValues.push(newValues);
        }
      });
    }

    txValues.forEach(async (entry, index) => {
      const address = Address.fromString(entry.address, this.network);
      ...
      const amounts = new Amounts(spent, received);
      const balance = received.isub(spent);
      await this.db.put(layout.g.encode(prefix, hash, time), amounts.toRaw());
      await this.updateAddressBalance(entry.address, balance);
    });
  }
  ...
  async unindexAddressBalance(meta, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      addressValues.forEach((entry) => {
        const findKey = _.findIndex(txValues, { 'address': entry.address });
        if(findKey > -1) {
          let newValues = {address: entry.address};
          txValues[findKey] = newValues;
        } else {
          let newValues = {address: entry.address};
          txValues.push(newValues);
    ...
    txValues.forEach(async (entry, index) => {
      ...
      await this.db.del(layout.g.encode(prefix, hash, time));
  ...
  async getAddressBalance(addr) {
    ...
    const check = await this.db.has(layout.Q.encode(prefix, hash));
    if(check)
      thisBalance = await this.db.get(layout.Q.encode(prefix, hash));

    return new BN(thisBalance);
  }
  ...
  async updateAddressBalance(addr, recent) {
    ...
    if(check) {
      ...
      if(recentBalance.isNeg()) {
        newBalance = prevBalance.isub(recentBalance.abs());
      } else {
        newBalance = prevBalance.iadd(recentBalance.abs());
      }
      
      await this.db.put(layout.Q.encode(prefix, hash), newBalance.toBuffer());
    } else {
      await this.db.put(layout.Q.encode(prefix, hash), recentBalance.toBuffer());
    }
...
function fromBigU64(num) {
  const data = bio.write(8);
  data.writeBigU64(num);
  return data.render();
}
