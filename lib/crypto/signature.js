...
Signature.fromCompact = function (buf) {
  ...
  assert(Buffer.isBuffer(buf) || buf instanceof Uint8Array, 'Argument is expected to be a Buffer');
  ...
  assert(i === 0 || i === 1 || i === 2 || i === 3, 'i must be 0, 1, 2, or 3');
...
Signature.parseDER = function (buf, strict) {
  ...
  assert(Buffer.isBuffer(buf) || buf instanceof Uint8Array, 'DER formatted signature should be a buffer');
  ...
  assert(!strict || length === buflength, 'Length byte should length of what follows');
...
Signature.prototype.hasDefinedHashtype = function () {
  if (!fixed.isNaturalNumber(this.nhashtype)) {
    return false;
  }
