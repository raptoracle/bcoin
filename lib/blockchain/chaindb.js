...
class ChainDB {
  ...
  async _save(entry, block, view) {
    ...
    this.put(layout.s.encode(roundMs(entry.time)), layout.H.encode(entry.height));
  ...
  async _reconnect(entry, block, view) {
    ...
    // Nearest hour timestamp->height index.
    this.put(layout.s.encode(roundMs(entry.time)), entry.height);
