...
class MempoolEntry {
  ...
  fromTX(tx, view, height) {
    ...
    const value = new BN(tx.getChainValue(view));
    const sigops = tx.getSigopsCount(view, flags);
    const size = tx.getSize();
    const priority = new BN(tx.getPriority(view, height, size)).toBigInt();
  ...
  getPriority(height) {
    ...
    let entryValue = new BN(this.value);
    let entrySize = new BN(this.size);
    //let priority = entryValue.imuln(delta);
    //priority.idiv(entrySize);
    //console.log(this.priority);
    //let result = priority.iaddn(this.priority);
    //const priority = (delta * this.value) / this.size;
    let priority = entryValue.imul(delta);
    priority.idiv(entrySize);
    priority.iadd(this.priority);

    //let result = this.priority + Math.floor(priority);
    //let result = this.priority + Math.floor(Number(priority));
