...
const consensus = require('../protocol/params/consensus');
...
class ChainDB {
  ...
  constructor(options) {
    ...
    if(this.mongo.enabled) {
      this.mdb = new MDB({
        dbhost: this.mongo.dbHost,
        dbname: this.mongo.dbName,
      });
    } else {
  ...
  start() {
    ...
    this.current = this.mongo.enabled ? this.mdb.batch() : this.db.batch();
  ...
  async getHeight(hash) {
    ...
    const height = this.mongo.enabled ? await this.mdb.getBlockHeightByHash(hash.toString('hex')) : await this.db.get(layout.h.encode(hash));
  ...
  async getHash(height) {
    ...
    if(this.mongo.enabled) {
      const hash = await this.mdb.getEntryHashByHeight(height);
      if(!hash)
        return null;
  ...
  async getEntryByHeight(height) {
    ...
    const hash = this.mongo.enabled ? await this.mdb.getEntryHashByHeight(height) : await this.db.get(layout.H.encode(height));
  ...
  async getEntryByHash(hash) {
    ...
    const raw = this.mongo.enabled ? await this.mdb.getEntryByHash(hash) : await this.db.get(layout.e.encode(hash));
  ...
  async getState() {
    const data = this.mongo.enabled ? await this.mdb.getTipHash() : await this.db.get(layout.R.encode());
  ...
  async getFlags() {
    const data = this.mongo.enabled ? await this.mdb.getChainOptions() : await this.db.get(layout.O.encode());
  ...
  async getStateCache() {
    ...
    const items = this.mongo.enabled ? await this.mdb.getStateCaches() : await this.db.range({
      gte: layout.v.min(),
      lte: layout.v.max(),
      values: true
    });
  ...
  saveDeployments() {
    const b = this.mongo.enabled ? this.mdb.batch() : this.db.batch();
  ...
  writeDeployments(b) {
    ...
    const bits = bw.render();

    if(this.mongo.enabled)
      this.mdb.setDeploymentBits(bits);
    else
      b.put(layout.D.encode(), bits);
  }
  ...
  async checkDeployments() {
    const raw = this.mongo.enabled ? await this.mdb.getDeploymentBits() : await this.db.get(layout.D.encode());
  ...
  async invalidateCache(bit, b) {
    if(this.mongo.enabled) {
      await this.mdb.invalidateStateCache();
    } else {
  ...
  async prune() {
    ...
    for (let i = start; i <= end; i++) {
      ...
      if(this.mongo.enabled) {
        this.mdb.deleteBcoinBlock(hash);
        await this.mdb.removeUndoCoins(layout.u.encode(hash));
      } else {
    ...
    try {
      ...
      if(this.mongo.enabled)
        await this.mdb.setChainOptions(flags.toRaw())
      else
  ...
  async getNextHash(hash) {
    return this.mongo.enabled ? await this.mdb.getNextHash(hash) : this.db.get(layout.n.encode(hash));
  }
  ...
  async getHashes(start = -1, end = -1) {
    ...
    if(this.mongo.enabled) {
      ...
      const startHash = await this.mdb.getBlockHashByHeight(start);
      const stopHash = await this.mdb.getBlockHashByHeight(end);
      console.log(start, end, [startHash, stopHash]);
  ...
  async getEntries() {
    if(this.mongo.enabled) {
      const entries = await this.mdb.getEntries();

      return entries.map((entry) => {
        return ChainEntry.fromRaw(entry.data);
      });
    }
    return this.db.values({
      gte: layout.e.min(),
  ...
  async getTips() {
    if(this.mongo.enabled) {
      const data = await this.mdb.getTips();
  ...
  async readCoin(prevout) {
    ...
    const raw = this.mongo.enabled ? await this.mdb.getCoins(layout.c.encode(hash, index)) : await this.db.get(layout.c.encode(hash, index));
  ...
  async hasCoins(tx) {
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if(this.mongo.enabled)
        if (await this.mdb.hasCoins(key))
          return true;
  ...
  async hasCoinsRef(tx) {
    for (let i = 0; i < tx.outputs.length; i++) {
      const key = layout.c.encode(tx.hash(), i);
      if(this.mongo.enabled)
        if (await this.mdb.hasCoins(key))
          return {spentId: tx.rhash(), spentIndex: i};
      if (await this.db.has(key))
        return {spentId: tx.rhash(), spentIndex: i};
    }
    return false;
  ...
  async getUndoCoins(hash) {
    const data = this.mongo.enabled ? await this.mdb.getUndoCoins(layout.u.encode(hash)) : await this.blocks.readUndo(hash);
  ...
  async getRawBlock(block) {
    ...
    return this.mongo.enabled ? await this.mdb.getRawBlock(hash) : this.blocks.read(hash);
  }
  ...
  async _save(entry, block, view) {
    ...
    if(this.mongo.enabled) {
      // Entry data.
      await this.mdb.saveEntry(hash, entry.height, entry.toRaw());
    ...
    if (!entry.isGenesis()) {
      if(this.mongo.enabled) {
        this.mdb.updateNextBlock(entry.prevBlock, hash);
      } else {
    ...
    if(!this.mongo.enabled) {
      // Nearest hour timestamp->height index.
      this.put(layout.s.encode(roundMs(entry.time)), fromU32(entry.height));
      this.put(layout.S.encode(entry.height), fromU32(roundMs(entry.time)));
    }
    ...
    if(this.mongo.enabled) {
      ...
      await this.mdb.setTipHash(this.pending.commit(hash));
    } else {
      // Commit new chain state.
  ...
  async _disconnect(entry, block) {
    ...
    // Nearest hour timestamp->height index.
    this.del(layout.s.encode(roundMs(entry.time)));
    this.del(layout.S.encode(entry.height));
  ...
  saveUpdates() {
    ...
    for (const update of updates) {
      ...
      if(this.mongo.enabled)
        this.mdb.saveStateCache(layout.v(bit, hash), update.toRaw());
      else
  ...
  async reset(block) {
    ...
    for (;;) {
      ...
      if (tip.hash.equals(entry.hash)) {
        if(this.mongo.enabled)
          await this.mdb.setTipHash(this.pending.commit(tip.hash));
        else
      ...
      if(this.mongo.enabled) {
        await this.mdb.removeTip(tip.hash);
        await this.mdb.saveTip(tip.prevBlock);
    
        this.mdb.deleteEntry(tip.hash);
      } else {
        ...
        this.del(layout.s.encode(roundMs(tip.time)));
        this.del(layout.S.encode(tip.height));
      }
      ...
      // Revert chain state to previous tip.
      if(this.mongo.enabled)
        await this.mdb.setTipHash(this.pending.commit(tip.prevBlock));
      else
  ...
  async _removeChain(hash) {
    ...
    for (;;) {
      ...
      if(this.mongo.enabled) {
        await this.mdb.removeTip(tip.hash);
        this.mdb.deleteEntry(tip.hash);
        this.mdb.deleteBcoinBlock(tip.hash);
      } else {
        this.del(layout.p.encode(tip.hash));
        this.del(layout.h.encode(tip.hash));
        this.del(layout.e.encode(tip.hash));
  ...
  async saveBlock(entry, block, view) {
    ...
    if(this.mongo.enabled)
      await this.mdb.saveBcoinBlock(entry, block);
    else
  ...
  async removeBlock(entry) {
    ...
    if(this.mongo.enabled)
      await this.mdb.deleteBcoinBlock(block.hash());
  ...
  async saveView(view) {
    for (const [hash, coins] of view.map) {
      for (const [index, coin] of coins.outputs) {
        ...
        if (coin.spent) {
          if(this.mongo.enabled) {
            await this.mdb.removeCoins(layout.c.encode(hash, index));
          } else {
            this.del(layout.c.encode(hash, index));
            this.put(layout.z.encode(hash, index), raw);
          }
        ...
        if(this.mongo.enabled)
          await this.mdb.saveCoins(layout.c.encode(hash, index), raw, coin, hash, index);
        else
  ...
  async connectBlock(entry, block, view) {
    ...
    if (!view.undo.isEmpty()) {
      if(this.mongo.enabled)
        await this.mdb.saveUndoCoins(layout.u.encode(hash), view.undo.commit());
      else
    ...
    if(this.mongo.enabled) {
      // Tip index. Block data is saved. Move Tip
      await this.mdb.removeTip(entry.prevBlock);
      await this.mdb.saveTip(hash);
    }
  ...
  async saveFlags() {
    ...
    if(this.mongo.enabled) {
      return await this.mdb.setChainOptions(flags.toRaw());
    } else {
...
async indexTX(tx, view, entry, index) {
  const hash = tx.hash();

  if (this.options.indexTX) {
    const meta = TXMeta.fromTX(tx, entry, index);

    await this.mdb.saveBcoinTx(entry, tx, meta);
  }
  ...
  if (!tx.isCoinbase()) {
    for (const { prevout } of tx.inputs) {
      ...
      await this.mdb.removeAddress(layout.C.encode(addr, prevout.hash, prevout.index));
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    await this.mdb.saveAddress(layout.C.encode(addr, hash, i), addr, hash, i);
  }
};
  ...
  async unindexTX(tx, view) {
    const hash = tx.hash();

    if (this.options.indexTX) {
      await this.mdb.deleteBcoinTx(hash);
    }
    ...
    if (!tx.isCoinbase()) {
      for (const { prevout } of tx.inputs) {
        ...
        await this.mdb.saveAddress(layout.C.encode(addr, prevout.hash, prevout.index));
      }
    }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      await this.mdb.removeAddress(layout.C.encode(addr, hash, i));
    }
  };
...
class ChainState {
  ...
  add(coin) {
    ...
    this.coin += 1;
    this.value = this.value.iadd(cv);
  }

  spend(coin) {
    let cv = new BN(coin.value);
    this.coin -= 1;
    this.value = this.value.isub(cv);
  }
  ...
  toRaw() {
    assert(BN.isBN(this.value), "chaindb.js toRaw(), value expected to be BN");
  ...
  static fromRaw(data) {
    ...
    state.value = new BN(br.readBigU64());
    return state;
  }
}
