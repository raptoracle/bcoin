...
const {fDIP0001Active, fDIP0003Active} = require('./common');
...
exports.verifyPOW = function verifyPOW(hash, bits, network) {
  ...
  //if (num.gt(target)) 
  //  return false;
  if(target.gt(network.pow.limit))
    return false;

  return true;
};

exports.verifyGenesisPOW = function verifyGenesisPOW(hash, nonce, network) {
  const genesis = network.genesis;
  const target = exports.fromCompact(genesis.bits);
  const num = new BN(hash, 'le');
  ...
  if (num.lte(target)) {
    console.log(num, target);
    console.log(genesis.nonce == nonce, genesis.nonce, nonce);
  }
...
exports.getCollateral = function getCollateral(height, network) {
  for (let i = 0; i < network.collaterals.length; i++) {
    let it = network.collaterals[i];
...
exports.getRewardPercentage = function getRewardPercentage(height, network) {
  for (let i = 0; i < network.rewardPercentages.length; i++) {
    let it = network.rewardPercentages[i];
		if(it.height === exports.INT_MAX || height <= it.height) {
...
exports.getSmartnodePayment = function getSmartnodePayment(network, height, blockValue, specialTxFees) {
  ...
  if(mnCount >= 10 || network.type === "main") {
    let percentage = exports.getRewardPercentage(height, network);
    let specialFeeReward = specialTxFees * network.futureRewardShare.smartnode;
...
exports.DIP0003Enabled = fDIP0003Active;
