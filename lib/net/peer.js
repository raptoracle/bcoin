...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger = this.options.logger.context('net.Peer');
  ...
  sendGetSporks() {
    ...
    if (this.sentGetSporks)
      return;

    this.sentGetSporks = true;
    this.send(new packets.GetSporksPacket());
  }
  ...
  maybeTimeout() {
    ...
    for (const [key, entry] of this.responseMap) {
      if (now > entry.timeout) {
        const name = packets.Types.typesByVal[key];
    ...
    if (this.syncing && this.loader && !this.options.isFull()) {
      //let headerTimeout = now + Peer.HEADERS_DOWNLOAD_TIMEOUT_BASE + Peer.HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (now - this.blockTime) / this.network.pow.targetSpacing;
      if (now > this.blockTime + Peer.CHAIN_SYNC_TIMEOUT) {
      //if (now > headerTimeout) {
      //if (now > this.time + this.network.pow.targetSpacing * (Peer.BLOCK_DOWNLOAD_TIMEOUT_BASE + Peer.BLOCK_DOWNLOAD_TIMEOUT_PER_PEER)) {
        this.error('Peer is stalling (headers) (block). (Syncing & loader, not full) peer.js, 1066');
        this.destroy();
        return;
      }
    }
    ...
    if (this.options.isFull() || !this.syncing) {
      for (const time of this.blockMap.values()) {
        if (now > time + Peer.BLOCK_TIMEOUT) {
          this.error('Peer is stalling (block). (Is full and not syncing) peer.js, 1075');
          this.destroy();
          return;
        }
      }
  ...
  async handleVersion(packet) {
    ...
    if (this.outbound) {
      if (!(this.services & services.NODE_NETWORK))
      ...
      if (this.options.spv) {
        if (!(this.services & services.NODE_BLOOM))
  ...
  async handleSpork(packet) {
    this.logger.debug(
      'Peer sent a spork packet (%s).',
      this.hostname());
      console.log(packet);
  }
...
Peer.BLOCK_DOWNLOAD_TIMEOUT_BASE = 1000000;
Peer.BLOCK_DOWNLOAD_TIMEOUT_PER_PEER = 500000;
Peer.HEADERS_DOWNLOAD_TIMEOUT_BASE = 15 * 60 * 1000000;
Peer.HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER = 1000;

Peer.BLOCK_TIMEOUT = 2 * 60 * 1000;

Peer.CHAIN_SYNC_TIMEOUT = 20 * 60 * 1000;
...
Peer.TIMEOUT_INTERVAL = 20 * 60 * 1000;
