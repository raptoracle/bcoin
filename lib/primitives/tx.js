...
class TX {
  ...
  checkSpecialTx(view, flags) {
    try {
      switch(this.type) {
        case TRANSACTION_PROVIDER_REGISTER:
          return this.checkProRegTx(view, flags);
        case TRANSACTION_PROVIDER_UPDATE_SERVICE:
          return this.checkProUpServTx(view, flags);
        case TRANSACTION_PROVIDER_UPDATE_REGISTRAR:
          return this.checkProUpRegTx(view, flags);
        case TRANSACTION_PROVIDER_UPDATE_REVOKE:
          return this.checkProUpRevTx(view, flags);
        case TRANSACTION_COINBASE:
          return this.checkCbTx(view, flags);
        case TRANSACTION_QUORUM_COMMITMENT:
          return this.checkLLMQCommitment(view, flags);
        case TRANSACTION_FUTURE:
          return this.checkFutureTx(view, flags);
      }
    } catch (e) {
      console.log(e);
      throw new VerifyError(this, 'invalid', 'failed-check-special-tx', 100);
    }
    throw new VerifyError(this, 'invalid', 'bad-tx-type-check', 10);
  }
  ...
  check(view, flags) {
    ...
    if(this.isSpecialTransaction())
      return this.checkSpecialTx(view, flags);
  ...
  checkProRegTx(view, flags) {
    const proRegTxPayload = Payload.serializeToJSON(this.extraPayload);
    return true;
  }

  checkProUpServTx(view, flags) {
    const proUpServTxPayload = Payload.serializeToJSON(this.extraPayload);
    return true;
  }

  checkProUpRegTx(view, flags) {
    const proUpRegTxPayload = Payload.serializeToJSON(this.extraPayload);
    return true;
  }

  checkProUpRevTx(view, flags) {
    const proUpRevTxPayload = Payload.serializeToJSON(this.extraPayload);
    return true;
  }

  checkCbTx(view, flags) {
    ...
    const cbtxPayload = Payload.serializeToJSON(this.extraPayload);

    if(!cbtxPayload)
      return false;

    if(cbtxPayload.version === 0 || cbtxPayload.version > 2)
      return false;

    return true;
  }

  checkLLMQCommitment(view, flags) {
    assert(this.db, 'Tx llmq check needs historical chain data');

    //let c = this.db.getEntryByHeight(2344);
      //console.log(c);
    
    //let network = Network.primary;
    //let network = Network.type;
    //console.log("checkLLMQCommitment", view, chain.network, chain.height);

    const qcTxPayload = Payload.serializeToJSON(this.extraPayload);
    //chain.getEntryByHash()
    if(!qcTxPayload) 
      return false; // bad-qc-payload

    //if(qcTxPayload.version === 0 || qcTxPayload.version > 1)
    //  return false; // bad-qc-version

    //if(qcTxPayload.height !== this.height)
      //return false; // bad-qc-height

    //if(!qcTxPayload.quorumHash)
    //  return false;

    //const pindexQuorum = chain.getEntryByHash(qcTxPayload.quorumHash);
    
    //if(!pindexQuorum)
    //  return false;

    //const pindexPrev = chain.getEntryByHeight(pindexQuorum.height);

    //if(qcTxPayload.quorumHash !== pindexPrev.hash)
    //  return false;

    return true;
  }

  checkFutureTx(view, flags) {
    const futureTxPayload = Payload.serializeToJSON(this.extraPayload);
    return true;
  }
  ...
  async verifyAsync(view, flags, pool, db) {
    if(!!db || this.db === null) {
      this.db = db;
    }
    try {
  ...
  isQuorumCommitment() {
    return this.type === registeredTransactionTypes.TRANSACTION_QUORUM_COMMITMENT;
  }
