...
class TX {
  ...
  _getAddressesValues(view) {
    ...
    for (const [addr, value] of Object.entries(oAddrs)) {
      addrs.push({[addr]: {received: value, spent: 0}});
    }

    for (const [addr, value] of Object.entries(iAddrs)) {
      const hasReceived = addrs.findIndex(e => e[addr] === addr);
      const received = hasReceived > -1 ? addrs[hasReceived].received : 0;
      addrs.push({[addr]: {received: received, spent: value}});
    }
