...
class Mempool extends EventEmitter {
  ...
  async open() {
    ...
    if (this.options.persistent) {
      ...
      for (const entry of entries) {
        ...
        if (this.options.fullIndex || this.options.indexAddress) {
  ...
  getSnapshot(verbose) {
    if(verbose)
      return this.getVerboseSnapshot();
  ...
  trackEntry(entry, view) {
    ...
    if (this.options.indexFull && view || this.options.indexAddress && view)
      this.indexEntry(entry, view);
  ...
  untrackEntry(entry) {
    ...
    if (this.options.indexFull || this.options.indexAddress)
...
class MempoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.indexFull != null) {
      assert(typeof options.indexFull === 'boolean');
      this.indexFull = options.indexFull;
    }
...
class MempoolCache {
  ...
  async wipe() {
    ...
    const prev = await this.chain.getEntry(tip);
    const cur = this.chain.tip.height;
    const batch = this.db.batch();
    ...
    if(!tip && !prev) {
      const dif = cur - prev;
      if(dif > policy.feeConstants.OLDEST_ESTIMATE_HISTORY) {
        batch.del(layout.F.encode());
      }
    }
