...
exports.encode = function encode(num, exp) {
  assert((Number.isSafeInteger(num) || typeof num === 'bigint' || typeof num === 'object' || BN.isBN(num)), 'Invalid number value.');
  //assert(, 'Invalid num, not BN');
  num = new BN(num);
  ...
  const mult = new BN(pow10(exp));

  //let lo = num % mult;
  let lo = num.imod(mult);
  let hi = num.isub(lo).idiv(mult);
  //let hi = (num - lo) / mult;
...
exports.decode = function decode(str, exp) {
  ...
  const decodedNum = new BN(sign * (hi * mult + lo));
 ...
 * @return {Boolean}
 */
exports.isNaturalNumber = function isNaturalNumber(value) {
  return (
    typeof value === 'number' &&
    isFinite(value) &&
...
exports.isUnsignedInteger = function isUnsignedInteger(number) {
  return (
