...
class ChainDB {
  ...
  start() {
    ...
    this.current = this.mongo ? this.mdb.batch() : this.db.batch();
  ...
  async getHeight(hash) {
    ...
    const height = this.mongo ? await this.mdb.getBlockHeightByHash(hash) : await this.db.get(layout.h.encode(hash));
  ...
  async getEntryByHeight(height) {
    ...
    const hash = this.mongo ? await this.mdb.getEntryHashByHeight(height) : await this.db.get(layout.H.encode(height));
  ...
  async getEntryByHash(hash) {
    ...
    const raw = this.mongo ? await this.mdb.getEntryByHash(hash) : await this.db.get(layout.e.encode(hash));
  ...
  async getState() {
    const data = this.mongo ? await this.mdb.getTipHash() : await this.db.get(layout.R.encode());
  ...
  async getFlags() {
    const data = this.mongo ? await this.mdb.getChainOptions() : await this.db.get(layout.O.encode());
  ...
  async getStateCache() {
    ...
    const items = this.mongo ? await this.mdb.getStateCaches() : await this.db.range({
      gte: layout.v.min(),
      lte: layout.v.max(),
      values: true
    });
  ...
  saveDeployments() {
    const b = this.mongo ? this.mdb.batch() : this.db.batch();
  ...
  async checkDeployments() {
    const raw = this.mongo ? await this.mdb.getDeploymentBits() : await this.db.get(layout.D.encode());
  ...
  async invalidateCache(bit, b) {
    if(this.mongo) {
      await this.mongo.invalidateStateCache();
    } else {
  ...
  async getNextHash(hash) {
    return this.mongo ? await this.mdb.getNextHash(hash) : this.db.get(layout.n.encode(hash));
  }
  ...
  async getEntries() {
    if(this.mongo) {
      ...
      return entries.map((entry) => {
        return ChainEntry.fromRaw(this.chain, entry.data);
      });
    }
    return this.db.values({
      gte: layout.e.min(),
  ...
  async readCoin(prevout) {
    ...
    const raw = this.mongo ? await this.mdb.getCoins(layout.c.encode(hash, index)) : await this.db.get(layout.c.encode(hash, index));
  ...
  async getUndoCoins(hash) {
    const data = this.mongo ? await this.mdb.getUndoCoins(layout.u.encode(hash)) : await this.blocks.readUndo(hash);
  ...
  async getRawBlock(block) {
    ...
    return this.mongo ? await this.mdb.getRawBlock(hash) : this.blocks.read(hash);
  }
