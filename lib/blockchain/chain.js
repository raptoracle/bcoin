...
const BN = require('bcrypto/lib/js/bn');
const validateTX = require('../protocol/validation');
...
class Chain extends AsyncEmitter {
  ...
  constructor(options) {
    ...
    this.slowMode = this.options.slowMode;
    this.sporksManager = this.options.sporksManager;

    this.db = new ChainDB(this.options);

    this.locker = new Lock(true, BufferMap);
    this.invalid = new LRU(100, null, BufferMap);
    this.state = new DeploymentState(this.network);
  ...
  async open() {
    ...
    if(this.sporksManager != null)
      await this.sporksManager.open();
  }
  ...
  async close() {
    ...
    if(this.sporksManager != null)
      await this.sporksManager.close();
  ...
  async verify(block, prev, flags) {
    ...
    if(this.network.type === 'main' && (prev.height + 1) <= 68589) {
      // architecture issues with DGW v1 and v2)
      let n1 = new BN(block.bits).toDouble();
      let n2 = new BN(bits).toDouble();

      if (Math.abs(n1-n2) > n1*0.5) {
        throw new VerifyError(block,
          'invalid',
          'bad-diffbits pre DGW bit failed to check',
          100);
      }
    } else {
      if (block.bits !== bits) {
  ...
  async getDeployments(time, prev) {
    ...
    const height = prev.height + 1;
    const state = new DeploymentState(this.network);
    ...
    const fStrictPayToScriptHash = (time >= consensus.BIP16_TIME);
    state.flags = fStrictPayToScriptHash ? Script.flags.VERIFY_P2SH : Script.flags.VERIFY_NONE;

    // Coinbase heights are now enforced (bip34).
    //if (this.network.block.bip34Enabled || height >= this.network.block.bip34height)
    //  state.bip34 = true;

    // Signature validation is now enforced (bip66).
    if (this.network.block.bip66Enabled || height >= this.network.block.bip66height)
      state.flags |= Script.flags.VERIFY_DERSIG;

    // CHECKLOCKTIMEVERIFY is now usable (bip65).
    if (this.network.block.bip65Enabled || height >= this.network.block.bip65height)
    ...
    //}
    if (this.network.block.bipCSVEnabled)
      state.flags |= Script.flags.VERIFY_CHECKSEQUENCEVERIFY;

    if(this.network.block.bip147Enabled)
      state.flags |= Script.flags.VERIFY_NULLDUMMY;

    if (await this.isActive(prev, deployments.csv)) {
      state.flags |= Script.flags.ENABLE_DIP0020_OPCODES;
    }
  ...
  async verifyInputs(block, prev, state) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (sigops > consensus.getMaxBlockSigOps()) {
        throw new VerifyError(block,
          'invalid',
          'bad-blk-sigops',
          100);
      ...
      if (i > 0) {
        ...
        let txFee = new BN(fee.toString(10));
        ...
        //reward += fee;
        reward = reward.iadd(txFee);
        //_fee += fee;
        _fee = _fee.iadd(txFee);

        //if (reward > consensus.MAX_MONEY) {
        if (!consensus.moneyRange(reward)) {
          throw new VerifyError(block,
            'invalid',
      ...
      if(i > 0 && tx.isSpecialTransaction()) {
        const [valid, reason, score] = await validateTX.check({
          db: this.db, 
    ...
    let bs = new BN(consensus.getBlockSubsidy(height));
    reward = reward.iadd(bs);

    let bc = new BN(block.getClaimed().toString(10));
    let bsf = reward.iadd(_fee);
    ...
    for (let i = 1; i < block.txs.length; i++) {
      const tx = block.txs[i];
      // we already verified the specials
  ...
  async _add(block, flags, id) {
    ...
    if (flags & common.flags.VERIFY_POW) {
      let verify = false;
      verify = block.verifyPOW(this.network);
      if(hash.equals(this.network.genesis.hash))
        verify = block.verifyGenesisPOW(this.network);

      if (!verify)
        throw new VerifyError(block, 'invalid', 'high-hash', 0);
    }
  ...
  getBehind() {
    return util.timeDifference(util.ms(), this.tip.time * 1000);
  }
  ...
  /**
   * Calculate the next target with DGW.
   * @param {ChainEntry} prev - Previous entry.
   * @returns {Promise} - returns Number
   * (target is in compact/mantissa form).
   */

  async getDGWTarget(prev) {
    const pow = this.network.pow;
    const nPastBlocks = pow.DGWBlocksAvg || 60;
    if (!prev || prev.height < nPastBlocks) {
      return pow.bits;
    }

    let bnPastTargetAvg = consensus.fromCompact(prev.bits);
    let pindex = prev;

    for (let nCountBlocks = 1; nCountBlocks <= nPastBlocks; nCountBlocks++) {
      let bnTarget = consensus.fromCompact(pindex.bits);
      if (nCountBlocks == 1) {
        bnPastTargetAvg = bnTarget;
      } else {
        // NOTE: that's not an average really...
        bnPastTargetAvg = bnPastTargetAvg.imuln(nCountBlocks).iadd(bnTarget).idivn(nCountBlocks + 1);
      }

      if(nCountBlocks != nPastBlocks) {
        assert(pindex); // should never fail
        pindex = await this.getPrevious(pindex);
      }
    }

    let actualTimespan = prev.time - pindex.time;
    let targetTimespan = nPastBlocks * pow.targetSpacing;

    if (actualTimespan < targetTimespan / 3 | 0)
      actualTimespan = targetTimespan / 3 | 0;

    if (actualTimespan > targetTimespan * 3)
      actualTimespan = targetTimespan * 3;

    bnPastTargetAvg.imuln(actualTimespan);
    bnPastTargetAvg.idivn(targetTimespan);

    if (bnPastTargetAvg.gt(pow.limit))
      return pow.bits;

    return consensus.toCompact(bnPastTargetAvg);
  }
  ...
  async getTarget(time, prev) {
    ...
    if ((prev.height + 1) >= pow.nPowDGWHeight) {
      return this.getDGWTarget(prev);
    }
...
class ChainOptions {
  ...
  fromOptions(options) {
    ...
    if (options.slowMode != null) {
      assert(typeof options.slowMode === 'boolean');
    ...
    if(options.sporksManager != null)
      this.sporksManager = options.sporksManager;

    if(options.mdb != null)
      this.mdb = options.mdb;

    if(options.mdbExclusive != null) {
      assert(typeof options.mdbExclusive === 'boolean');
      this.mdbExclusive = options.mdbExclusive;
    }
...
class DeploymentState {
  ...
  constructor(network) {
    ...
    this.bip34 = network.block.bip34Enabled;
  }
