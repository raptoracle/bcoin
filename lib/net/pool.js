...
const packetTypes = packets.Types.typesByIndex;
const packetCmds = packets.Types.typesByVal;
const packetsUnsupported = packets.Types.unsupportedTypes;
...
class Pool extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger = this.options.logger.context('net.Pool');
  ...
  isSyncable(peer) {
    ...
    if (!(peer.services & services.NODE_NETWORK))
      return false;
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.BLOCKTXN:
        ...
        break;
      case packetTypes.SENDDSQUEUE:
        await this.handleSendDSQueue(peer, packet);
        break;
      case packetTypes.GETSPORKS:
        await this.handleGetSporks(peer, packet);
        break;
      case packetTypes.SPORK:
        await this.handleSpork(peer, packet);
        break;
  ...
  async _handleInv(peer, packet) {
    ...
    if (unknown !== -1) {
      if(packetsUnsupported.includes(packetCmds[unknown])) {
        //we dont really need to listen to this
        //this.logger.warning(
        //  'Peer sent an unknown inv type,cmd: %d,%s (%s).',
        //  unknown, packetCmds[unknown], peer.hostname());
      } else {
        this.logger.warning(
          'Peer sent an unknown inv type,cmd: %d,%s (%s).',
          unknown, packetCmds[unknown], peer.hostname());
...
*/
  ...
  logStatus(block) {
    if (this.chain.height % 20 === 0) {
      this.logger.debug('Status:'
        + ' time=%s (%s behind) height=%d progress=%s'
        + ' orphans=%d active=%d'
        + ' target=%s peers=%d',
        util.date(block.time),
        this.chain.getBehind(),
        this.chain.height,
  ...
  async handleSendDSQueue(peer, packet) {
    this.logger.info(
      'Peer sent senddsq packet: (%s).',
      peer.hostname());
    ;
  }

  /**
   * Handle `getsporks` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {GetSporksPacket} packet
   */

    async handleGetSporks(peer, packet) {
      ;
    }
   ...
   * @private
   * @param {Peer} peer
   * @param {SporkPacket} packet
   */


  async handleSpork(peer, packet) {
    this.logger.info('Received spork packet from peer (%s).', peer.hostname());
    if(this.sporks && this.sporkManager != null)
      this.sporkManager.process(packet);
  }
  ...
  getBlock(peer, hashes) {
    ...
    //let checkSize = peer.height - this.chain.tip.height;

    //if (peer.blockMap.size > checkSize) {
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (this.spv) {
      this.requiredServices |= common.services.NODE_BLOOM;
      this.services &= ~common.services.NODE_NETWORK;
      this.noRelay = true;
      this.checkpoints = true;
      this.compact = false;
      this.bip37 = false;
    ...
    if (this.bip37)
      this.services |= common.services.NODE_BLOOM;
    ...
    if(options.spork != null) {
      assert(typeof options.spork === 'boolean');
      this.spork = options.spork;
    }
