...
class ConfirmStats {
  ...
  estimateMedian(target, needed, breakpoint, greater, height, result) {
    ...
    this.logger.debug('FeeEst: %d %s%.0f%% decay %.5f: feerate: %g from (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)', 
      target, greater ? ">" : "<", breakpoint, decay, median, _passBucket.start, _passBucket.end,
    ...
    if(target >= 1 && target <= this.longStats.maxConfirms) {
      ...
      if(checkShorterHorizon) {
        if(target > this.feeStats.maxConfirms) {
          let medMax = this.feeStats.estimateMedian(this.feeStats.maxConfirms, SUFFICIENT_TXS_SHORT, successThreshold, true, this.bestHeight);
        ...
        if(target > this.shortStats.maxConfirms) {
          let shortMax = this.shortStats.estimateMedian(this.shortStats.maxConfirms, SUFFICIENT_TXS_SHORT, successThreshold, true, this.bestHeight);
